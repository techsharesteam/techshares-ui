...
var TechSharesApi = require("./TechSharesApi");
...
class Apis {
    ...
    connect() {
        ...
        this.init_promise = this.ws_rpc.login(rpc_user, rpc_password).then(() => {
            this._db_api = new TechSharesApi(this.ws_rpc, "database");
            if (window) window.$db_api = this._db_api;
            this._network_api = new TechSharesApi(this.ws_rpc, "network_broadcast");
            this._history_api = new TechSharesApi(this.ws_rpc, "history");
