...
try {
    ...
    console.log("WARN: Will be unable to filter THS 1.0 wallet imports, did not find assets/bts_genesiskeys_bloom.dat", e)
}

/**
    This should only be applied to a THS 1.0 export file taken on the 
    discontinued chain. Any public key string or address (all 5 formats) carried
    over to the THS 2.0 genesis block will be in this filter.
...
export default class GenesisFilter {
    ...
    filter( account_keys, status ) {
        if( ! this.isAvailable() ) {
            console.log("WARN: Missing bloom filter for THS 0.9.x wallets")
            ...
            try {
                ...
                for(var a = 0; a < account_keys.length; a++) {
                    ...
                    for(var k = keys.encrypted_private_keys.length - 1; k >= 0; k--) {
                        ...
                        var addresses = key_utils.addresses(key, 'THS')
