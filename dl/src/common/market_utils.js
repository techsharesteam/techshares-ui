...
class MarketUtils {
    ...
    static parseOrder(order, base, quote, invert = false) {
        ...
        if (order.debt) {
            if (invert) {
                // Price in USD/THS, amount should be in THS, value should be in USD, debt is in USD
                // buy is in USD, sell is in THS
                // quote is USD, base is THS
                value = order.debt / quotePrecision;
                amount = this.limitByPrecision(value / price.full, base);
            } else {
                // Price in THS/USD, amount should be in USD, value should be in THS, debt is in USD
                // buy is in USD, sell is in THS
                // quote is USD, base is THS
