...
// Filter THS 0.9.2+ import keys export file so that it will include only private keys
// that may be found in the THS 2.0 genesis block. 
...
// The bloom filter was created with a genesis containing THS prefixed keys.  Create
...
chain_config.address_prefix = "THS"
...
fs.readFile('bloom.dat', function (err, data) {
    ...
    function in_bloom(str) {
        // echo -n '0:THS87mopaNqLDjT1BvzqQR3QjWzWSTgkWnMcwt5sqxHuavCBi1s3m'|sha256sum
        // str = 'THS87mopaNqLDjT1BvzqQR3QjWzWSTgkWnMcwt5sqxHuavCBi1s3n' // should be found
        // str = 'THS4A43UCoWz1F5vqLxX3LLoGHKs1GzCGRjZ' // should be found
    ...
    stdin.on('end', function () {
        ...
        for(var a = 0; a < wallet.account_keys.length; a++) {
            ...
            for(var k = keys.encrypted_private_keys.length - 1; k >= 0; k--) {
                ...
                if( /^GPH/.test(key) ) key = "THS" + key.substring(3)
